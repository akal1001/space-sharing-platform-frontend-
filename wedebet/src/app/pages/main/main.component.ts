...
export class MainComponent implements OnInit {
  ...
  private initializeLocationData(): void {
    ...
    var IsLocationsChangedByAddminre = sessionStorage.getItem("locChanged");

    if (IsLocationsChangedByAddminre === "yes") {
  ...
  private async loadInitialHouseData(): Promise<void> {
    try {
      ...
      const loadRequest = { maxId: maxIdData.data, location: locationData.data, token:userToken };
  ...
  private fetchHouseData(loadRequest: { maxId: string; location: string; token:string }): void {
    //this.isLoading = true;
    ...
    this.houseDataService.fetchHouseDataByLocation(loadRequest).subscribe({
      ...
      error: (error) => {
       ...
       // alert("")
        this.indexedDbService.cachedData.clear();
  ...
  private GetUseFavePostsHouseId() {
    let userSavedhouseIds: string[] = [];
    this.houseDataService.GetAllMySelectionPost(1, 50).subscribe({
      next: (response) => {
        for (var i = 0; i < response.data.length; i++) {
          console.log("ids: " + response.data[i].house.houseId);
          userSavedhouseIds.push(response.data[i].house.houseId)
        }
        if(userSavedhouseIds.length > 0){
          this.indexedDbService.saveDataAndEncrypted('faveIds', userSavedhouseIds);
          //sessionStorage.setItem('faveIds', JSON.stringify(userSavedhouseIds))
        }
      }, error(err) {

      },
    })
    console.log("user saved house ids: " + userSavedhouseIds)
